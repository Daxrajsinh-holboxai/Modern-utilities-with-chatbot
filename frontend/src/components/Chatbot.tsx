...
const socket = io(B_url, { autoConnect: false });
...
const Chatbot = () => {
  ...
  useEffect(() => {
    ...
    const handleReply = (data: { message: string }) => {
      if (!receivedMessages.has(data.message)) {
        setChat((prev: Message[]) => [...prev, { sender: "owner", message: data.message }]);
        setReceivedMessages((prev) => new Set([...prev, data.message]));
      }
    };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setChat((prev: Message[]) => [...prev, { sender: "user", message }]);
      setMessage("");
    } catch (error) {
  ...
  const getMessageClasses = (sender: string): string => {
    if (sender === "user") return "bg-blue-100 self-end text-blue-900";
    if (sender === "owner") return "bg-green-100 self-start text-green-900";
